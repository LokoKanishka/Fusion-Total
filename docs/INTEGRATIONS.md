...
Qué refuerza: doble control de aprobación (`approval_token` + `approval_ts` + `approval_scope`) + firma HMAC (`approval_sig`) y ticket obligatorio para `sensitive` (`approval_change_ticket`) vía `config/mcp_approval_policy.json`
