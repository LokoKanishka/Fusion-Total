...
Qué refuerza: doble control de aprobación (`approval_token` + `approval_ts` + `approval_scope`) + firma HMAC (`approval_sig`) + ticket obligatorio para `sensitive` (`approval_change_ticket`) + separación de funciones (`approved_by` distinto de `requested_by`) vía `config/mcp_approval_policy.json`
