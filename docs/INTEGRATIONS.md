...
Qué refuerza: doble control de aprobación (`approval_token` + `approval_ts` + `approval_scope`, y justificación para `sensitive`) + firma HMAC (`approval_sig`) vía `config/mcp_approval_policy.json`
